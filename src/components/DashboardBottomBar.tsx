...
export default function DashboardBottomBar() {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 bg-[#0a0e17] border-t border-[#00f0ff]/20 z-50">
      <div className="flex justify-around items-center h-16">
        {navItems.map((item, index) => (
          <Link key={index} to={item.href} className="flex flex-col items-center justify-center w-full h-full">
            <div
              className={`flex flex-col items-center justify-center ${
                isActive(item.href) ? "text-[#00f0ff]" : "text-gray-400"
              }`}
            >
              {isActive(item.href) ? (
                <motion.div initial={{ scale: 0.8 }} animate={{ scale: 1 }} className="flex flex-col items-center">
                  ...
                  <motion.div layoutId="bottomNav" className="absolute bottom-0 w-12 h-1 bg-[#00f0ff] rounded-t-full" />
                </motion.div>
              ) : (
